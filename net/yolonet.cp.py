    ...
    def load_weights(self, yolo_weight_file, weight_num):
        ...
        while layer_index < weight_num:
            ...
            if isinstance(layer, Dense):
                input_size = layer.get_weights()[0].shape[0]
                weights = np.ndarray(
                    shape=(input_size, layer.units),
                    dtype='float32',
                    buffer=weights_file.read(input_size * layer.units * 4)
                )
                biases = np.ndarray(
                    shape=(layer.units,),
                    dtype='float32',
                    buffer=weights_file.read(layer.units * 4))
                layer.set_weights([weights, biases])
            if isinstance(layer, Conv2D) or isinstance(layer, LocallyConnected2D):
